...
def attach_training_logger(
    ...
    def log_training_loss(engine: engine.Engine):
        ...
        logger.info(f'epoch[{epoch}] - iteration[{idx_in_epoch}/{epoch_length}] loss: {output:.3f}')
...
def attach_model_checkpoint(trainer: engine.Engine, models: Dict[str, nn.Module], name: str):
